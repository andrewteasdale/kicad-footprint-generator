...
name = "Harwin_Gecko-{pn}_2x{n:02}_P{p:.2f}mm_Vertical"
...
if __name__ == '__main__':
    ...
    for pins in pincount:
        ...
        for i in range(rows):
            footprint.append(PadArray(pincount=pins, start=[0,i*pitch], initial=((i*pins)+1), increment=1, x_spacing=pitch, type=Pad.TYPE_THT, shape=Pad.SHAPE_CIRCLE, size=pad_dia, drill=drill, layers=['*.Cu','*.Mask']))
        ...
        footprint.append(Arc(center=[x1+r,y1+r],start=[x1,y1+r],angle=90, layer='F.Fab'))
        footprint.append(Arc(center=[x2-r,y1+r],start=[x2-r,y1],angle=90, layer='F.Fab'))
        footprint.append(Arc(center=[x2-r,y2-r],start=[x2,y2-r],angle=90, layer='F.Fab'))
        footprint.append(Arc(center=[x1+r,y2-r],start=[x1+r,y2],angle=90, layer='F.Fab'))
        
		#add p1 marker
        px = -2
        py = -2
        m = 0.25
        footprint.append(Circle(radius=m,center=[px,py]))
